...
def process_result_file(file, store):
  ...
  store.has_ans_em.append(result['exact'])
  store.has_ans_f1.append(result['f1'])
...
def average_results(filelist):
  ...
  for line in open(filelist, 'r').readlines():
    ...
    if len(files) == 0:
      print(f'No results file found on {line}')
      continue
    for f in files:
      process_result_file(os.path.join(line, f), file_metric)
